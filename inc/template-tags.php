...
if (!function_exists('bootscore_category')) :
  function bootscore_category() {
    ...
    if ('post' === get_post_type()) {
      ...
      $categories_list = get_the_category_list(esc_html__(', ', 'ufl_stamats'));
      if ($categories_list) {
...
if (!function_exists('bootscore_author')) :
  ...
  function bootscore_author() {
    $byline = sprintf(
      esc_html_x('by %s', 'post author', 'ufl_stamats'),
      ...
      comments_popup_link(
        sprintf(
          wp_kses(
            ...
            __('Leave a Comment', 'ufl_stamats'),
            array(
              'span' => array(
                'class' => array(),
              ),
...
if (!function_exists('bootscore_comment_count')) :
  ...
  function bootscore_comment_count() {
    if (!post_password_required() && (comments_open() || get_comments_number())) {
      ...
      /* translators: %s: Name of current post. Only visible to screen readers. */
      // comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'ufl_stamats' ), get_the_title() ) );
      comments_popup_link(sprintf(__('Leave a comment', 'ufl_stamats'), get_the_title()));
  ...
  function bootscore_tags() {
    ...
    if ('post' === get_post_type()) {
      ...
      if ($tags_list) {
        ...
        printf('<div class="tags-links mt-2">' . esc_html__('Tagged %1$s', 'ufl_stamats') . '</div>', $tags_list); // WPCS: XSS OK.
      }
    }
  }
