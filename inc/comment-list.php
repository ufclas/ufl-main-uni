    ...
    if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) : ?>
      ...
      <li class="bg-danger" id="comment-<?php comment_ID(); ?>" <?php comment_class('media'); ?>>
        <div class="comment-body">
          <?php _e('Pingback:', 'ufl_stamats'); ?> <?php comment_author_link(); ?> <?php edit_comment_link(__('Edit', 'ufl_stamats'), '<span class="edit-link">', '</span>'); ?>
        </div>
          ...
          <div class="comment-content">
            <div class="card">
              <div class="card-body">
                ...
                <div class="mt-0"><?php printf(__('%s <span class="says d-none">says:</span>', 'ufl_stamats'), sprintf('<h3 class="">%s</h3>', get_comment_author_link())); ?>
                </div>

                <div class="small comment-meta text-muted mb-2">
                  <time datetime="<?php comment_time('c'); ?>">
                    <?php printf(_x('%1$s at %2$s', '1: date, 2: time', 'ufl_stamats'), get_comment_date(), get_comment_time()); ?>
                  </time>
                  <?php edit_comment_link(__('Edit', 'ufl_stamats'), '<span class="edit-link">', '</span>'); ?>
                </div>


                <?php if ('0' == $comment->comment_approved) : ?>
                  <p class="comment-awaiting-moderation"><?php _e('Your comment is awaiting moderation.', 'ufl_stamats'); ?></p>
                <?php endif; ?>
...
function wp44138_change_comment_form_cookies_consent($fields) {
  ...
  $fields['cookies'] = '<p class="comment-form-cookies-consent custom-control form-check mb-3">' .
    ...
    '<label for="wp-comment-cookies-consent" class="form-check-label">' . __('Save my name, email, and website in this browser for the next time I comment.', 'ufl_stamats') . '</label>' .
    '</p>';
  return $fields;
}
